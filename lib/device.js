...
var options = {
  ...
  testcasesin:  path.normalize(config.ua_parser.dir + '/test_resources/' + config.ua_parser.test_resources.device),
  testcasesout: path.normalize(config.output.dir + '/' + config.ua_parser.test_resources.device),
  listfile: "device.csv",
  logfile:  "device.log.csv",
  console:   false,
  family:    true,
  debug:     true,
  swapdebug: false,
...
function main(opts) {
  ...
  opts.fields = [ 'debug', 'brand', 'model', 'swapdebug', 'family', 'user_agent_string' ];
  opts.fieldsBroken = ['debug', 'family_is', 'family_was', 'brand_is', 'brand_was', 'model_is', 'model_was','user_agent_string'];
