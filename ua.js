...
var
  uaParser = require(config.ua_parser + '/ua'),
...
var options = {
  file: "useragents.txt",
  testcasesin:  __dirname + '/' + config.test_resources.dir + '/' + config.test_resources.ua,
  testcasesout: __dirname + '/' + config.output.dir + '/' + config.test_resources.ua,
  listfile: "ua.csv",
  logfile:  "ua.log",
  console: false,
  family: true,
...
function main(opts) {
  ...
  parser = uaParser.makeParser(regexes.user_agent_parsers);
...
if (require.main === module) {
  var errmsg = 'Usage: (e.g.)\n' + 
    '  node ua.js file:uas.txt testcases:true';
